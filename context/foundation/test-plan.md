...
- **Cost × Signal:** Every test must answer: _what is the cheapest test that gives a real signal for this risk?_ No e2e tests unless cheaper layers fail.
...
### Component Logic Testing (Phase 2)

- **Pattern:** Extract complex data transformation hooks (e.g. `useMemo`) into pure functions in `src/lib/`.
- **Location:** `src/lib/*.ts` (pure functions) tested by `src/lib/*.test.ts`.
- **Avoid:** Do not use `jsdom` or `@testing-library/react`. Keep React components as thin view layers.
- **Reference:** `src/lib/snapshot-logic.ts` and `src/components/NetWorthDashboard.tsx`.
